...
type EnumDefinition struct {
	...
	PrefixTypeName           bool
	PrefixedSchemaEnumValues map[string]string
}
...
func (e *EnumDefinition) GetValues() map[string]string {
	...
	if e.PrefixedSchemaEnumValues == nil {
		...
		e.PrefixedSchemaEnumValues = newValues
	}
	return e.PrefixedSchemaEnumValues
}
...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	if t == "" || t == "object" {
		...
		if len(path) > 1 { // handle additional type only on non-toplevel types
			...
			// cause conflicts - see https://github.com/lkarlslund/oapi-codegen/issues/832
			var typeName string
